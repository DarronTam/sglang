...
def _pack_zeus_linear_weight(w_cpu, dtype=torch.bfloat16):
    """Pack an (out, in) CPU weight for Zeus aten::linear."""
...
def test_moe_block_full(cfg, num_tokens=16, seed=42):
    ...
    print(f"        REF 走 REF_DEVICE 预计算 gate/shared；Zeus 路径在 Zeus 上重算，")
    print(f"        避免 CUDA/CPU 到 Zeus 的 unsupported direct copy。")
    ...
    # ── Zeus 路径 ──
    # Zeus 不支持从 CUDA tensor 直接 copy 到 "zeus"。这里和生产路径保持一致：
    # gate Linear 与 shared experts MLP 都在 Zeus 上重新计算。
    x_z = x_bf16.to("zeus")
    gate_w_z = _pack_zeus_linear_weight(gate_w_bf16)
    sh_gu_z = _pack_zeus_linear_weight(sh_gu_bf16)
    sh_dp_z = _pack_zeus_linear_weight(sh_dp_bf16)
    corr_bias_z = corr_bias_fp32.to("zeus")
    w13_z = w13_bf16.to("zeus")
    w2_z = w2_bf16.to("zeus")

    with torch.no_grad():
        # 0a) gate Linear: bf16 × bf16 → bf16, 进 topk 前 .float()
        router_logits_z = torch.nn.functional.linear(x_z, gate_w_z).float()
        # 0b) shared_experts MLP: gate_up → silu_and_mul → down
        sh_gu_out_z = torch.nn.functional.linear(x_z, sh_gu_z)
        sh_silu_z = sgl_kernel_zeus.silu_and_mul(sh_gu_out_z)
        shared_out_z = torch.nn.functional.linear(sh_silu_z, sh_dp_z)
