...
class ChatCompletionMessageGenericParam(BaseModel):
    role: Literal["system", "assistant", "tool", "function", "developer", "observation"]
    ...
    def _normalize_role(cls, v):
        if isinstance(v, str):
            v_lower = v.lower()
            if v_lower not in {"system", "assistant", "tool", "function", "developer", "observation"}:
                raise ValueError(
                    "'role' must be one of 'system', 'developer', 'assistant', 'tool', 'observation' or 'function' (case-insensitive)."
                )
            return v_lower
