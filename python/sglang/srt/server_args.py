...
SAMPLING_BACKEND_CHOICES = {"flashinfer", "pytorch", "ascend", "zeus"}
LOAD_FORMAT_CHOICES = [
    "auto",
    "pt",
    "safetensors",
...
class ServerArgs:
    ...
    def _handle_piecewise_cuda_graph(self):
        ...
        if self.pp_size > 1:
            self.disable_piecewise_cuda_graph = True
        # 6. Non-CUDA hardware (AMD, NPU, CPU, MPS, XPU, Zeus, etc.)
        if is_hip() or is_npu() or is_cpu() or is_mps() or is_xpu() or is_zeus():
            self.disable_piecewise_cuda_graph = True
        # 7. MoE A2A backend
    ...
    def _handle_zeus_backends(self):
        if is_zeus():
            from sglang.srt.hardware_backend.zeus.utils import (
    ...
    def _handle_linear_attn_backend(self):
        ...
        if (decode == "zeus") != is_zeus():
            raise ValueError(
                "--linear-attn-backend zeus requires --device zeus"
            )
