...
class EBNFComposer:
    ...
    def build_ebnf(
        ...
        ebnf_lines = [
            f"root ::= {root_rule}",
            "function_call ::= "
            + " | ".join([f"call_{hash_string_to_short_hex(tool.function.name) if call_rule_fmt else tool.function.name}" for tool in tools]),
        ]
        ...
        # =================================================================
        call_template = (
            f"call_{{namehash}} ::= {call_rule_fmt}"
            if call_rule_fmt
        ...
        for tool in tools:
            ...
            namehash = hash_string_to_short_hex(tool_name) if call_rule_fmt else tool_name
            ebnf_lines.append(
                call_template.format(
                    name=tool_name, arguments_rule=f"arguments_{namehash}",
                    namehash=namehash,
                )
            )
            ebnf_lines.append(f"arguments_{namehash} ::= {arguments_rule}")
...
def hash_string_to_short_hex(input_string: str) -> str:
    encoded_string = input_string.encode('utf-8')
    hasher = hashlib.sha256(encoded_string)
    full_hex_hash = hasher.hexdigest()
    return full_hex_hash[:16]
