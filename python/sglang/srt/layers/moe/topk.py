...
class TopK(MultiPlatformOp):
    ...
    def forward_zeus(
        ...
        *,
        ...
        # zeus_biased_grouped_topk covers grouped + correction_bias + renormalize
        # + no fused shared experts. scoring_func is intentionally NOT checked:
        # the production select_experts (this file ~line 1087) dispatches to
        # biased_grouped_topk whenever correction_bias is not None regardless
        # of scoring_func, and biased_grouped_topk_impl hardcodes sigmoid
        # internally — so TopKConfig.scoring_func is semantically inert on this
        # path (see also the "not used for now" comment near line 262).
        #
        # Hard-assert on misconfig rather than fall back to CPU: silent
        # fallback during Zeus bring-up hides which configs still need kernel
        # coverage. If a future config genuinely cannot run on Zeus, extend
        # sgl_kernel_zeus.biased_grouped_topk (or add a sibling kernel) and
        # broaden this predicate.
        assert (
            self.topk_config.use_grouped_topk
            and self.topk_config.renormalize
            and self.topk_config.num_fused_shared_experts == 0
            and self.topk_config.correction_bias is not None
        ), (
            "TopK.forward_zeus: config not covered by zeus_biased_grouped_topk. "
            f"Got use_grouped_topk={self.topk_config.use_grouped_topk}, "
            f"renormalize={self.topk_config.renormalize}, "
            f"num_fused_shared_experts={self.topk_config.num_fused_shared_experts}, "
            f"correction_bias={'set' if self.topk_config.correction_bias is not None else 'None'}. "
            "Required: grouped=True, renorm=True, shared_experts=0, bias=set. "
            "Extend sgl_kernel_zeus / broaden this predicate rather than "
            "silently falling back to CPU."
        )

        from sgl_kernel_zeus import biased_grouped_topk as zeus_biased_grouped_topk

        topk_weights, topk_ids = zeus_biased_grouped_topk(
            router_logits,
            self.topk_config.correction_bias,
            num_expert_group=self.topk_config.num_expert_group,
            topk_group=self.topk_config.topk_group,
            topk=self.topk_config.top_k,
            num_fused_shared_experts=0,
            routed_scaling_factor=self.topk_config.routed_scaling_factor,
            apply_routed_scaling_factor_on_output=self.topk_config.apply_routed_scaling_factor_on_output,
        )
