...
from sglang.srt.utils import add_prefix, ceil_align, is_cuda, is_hip, is_npu, is_zeus
...
class Indexer(MultiPlatformOp):
    def __init__(
        ...
        if _is_cuda:
            ...
            # Non-CUDA accelerators (NPU / Zeus / CPU) — SM count is a CUDA
            # concept; the dual-stream PP overlap path is also CUDA-only.
            self.sm_count = None
            self.half_device_sm_count = None
    ...
    def forward_zeus(
        ...
        Builds q/k for the NSA lightning indexer, scores against the
        compressed K cache, and returns top-k absolute kv positions for
        the downstream sparse MLA backend.

        Patterned after ``forward_cuda`` but:
          * uses ``sgl_kernel_zeus`` kernels (act_quant / mqa-logits / rope);
          * no multi-stream overlap (Zeus single-stream first cut);
          * no fp8 fast-path skip — every token goes through the full pipeline.
        """
        from sgl_kernel_zeus import act_quant_zeus  # quantize bf16 -> fp8 + scale
        from sgl_kernel_zeus import fp8_mqa_logits_zeus  # paged MQA logits
        from sgl_kernel_zeus import rotary_embedding as zeus_rope
        ...
        # Tuple activations (fp8 + scale) are a CUDA fast-path artifact; on
        # Zeus we always work with the bf16 representative for shape/device.
        x_meta = x[0] if isinstance(x, tuple) else x
        bs = q_lora.shape[0]

        # ---------- 1. q: wq_b -> split nope/rope -> RoPE on rope half ----------
        q = self.wq_b(q_lora)[0].view(bs, self.n_heads, self.head_dim)
        q_pe, q_nope = torch.split(
            q,
            [self.rope_head_dim, self.head_dim - self.rope_head_dim],
            dim=-1,
        )
        # Cache cos/sin per ForwardBatch so we don't re-lookup per layer.
        if not hasattr(forward_batch, "zeus_indexer_sin_cos_cache"):
            cos_sin = self.rotary_emb.cos_sin_cache[positions]
            forward_batch.zeus_indexer_sin_cos_cache = cos_sin
        else:
            cos_sin = forward_batch.zeus_indexer_sin_cos_cache
        q_pe = zeus_rope(
            q_pe, cos_sin, is_neox=self.rotary_emb.is_neox_style,
        )
        q = torch.cat([q_pe, q_nope], dim=-1)

        # ---------- 2. k: wk(x) -> k_norm -> split nope/rope -> RoPE ----------
        x_flat = x_meta.view(-1, self.hidden_size)
        weights = self.weights_proj(x_flat.float())[0].to(torch.bfloat16)
        k_proj = self.wk(x_flat)[0]
        ...
        k_pe = zeus_rope(
            k_pe, cos_sin, is_neox=self.rotary_emb.is_neox_style,
        )
        k = torch.cat([k_pe, k_nope], dim=-1).unsqueeze(1)  # [bs, 1, head_dim]

        # ---------- 3. fp8 quant q + persist index-K cache ----------
        q_fp8, q_scale = act_quant_zeus(q, self.block_size, self.scale_fmt)
        weights = weights * q_scale * self.softmax_scale
        self._store_index_k_cache(
            forward_batch=forward_batch,
            layer_id=layer_id,
            key=k.squeeze(1),
            act_quant=act_quant_zeus,
        )

        # ---------- 4. paged MQA logits over the full kv range ----------
        # Returns [total_tokens, max_kv_len] fp32 logits with -inf masks past
        # each per-row cache_seqlen.
        logits = fp8_mqa_logits_zeus(
            q_fp8=q_fp8,
            q_scale=q_scale,
            weights=weights,
            ...
            cache_seqlens=metadata.attn_metadata.cache_seqlens_int32,
        )

        # ---------- 5. top-k -> [tokens, index_topk] int32 ----------
        max_kv = logits.shape[-1]
        topk = min(self.index_topk, max_kv)
        topk_result = logits.topk(topk, dim=-1).indices.to(torch.int32)
        if topk < self.index_topk:
            pad = torch.full(
                (topk_result.shape[0], self.index_topk - topk),
                -1,
                dtype=torch.int32,
                device=topk_result.device,
            )
            topk_result = torch.cat([topk_result, pad], dim=-1)

        # ---------- 6. PAGED transform (token-index -> page-index) ----------
        # Lazy import to avoid a circular dep (nsa_backend imports Indexer).
        ...
        if metadata.topk_transform_method == TopkTransformMethod.PAGED:
            topk_result = metadata.topk_transform(
                topk_result,
                self.index_topk,
                ks=metadata.attn_metadata.cache_seqlens_int32,
