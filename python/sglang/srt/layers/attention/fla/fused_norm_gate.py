...
class FusedRMSNormGated(MultiPlatformOp):
    def __init__(
        self,
        hidden_size: int,
    ...
    def forward_zeus(
        ...
        # TODO: sgl_kernel_zeus.rms_norm_gated currently ignores
        # weight / bias / activation / residual / prenorm / residual_in_fp32.
        # All in-tree callers (glm5_next, kimi_linear, qwen3_next) invoke as
        # (x, g) only, so it's functionally OK today but precision-incomplete —
        # extend the kernel before exercising the residual / prenorm path.
        from sgl_kernel_zeus import rms_norm_gated as zeus_rms_norm_gated

        return zeus_rms_norm_gated(x, g, eps=self.eps)
