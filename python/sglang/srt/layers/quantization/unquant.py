...
class UnquantizedFusedMoEMethod(FusedMoEMethodBase, MultiPlatformOp):
    ...
    def forward_zeus(
        ...
        from sgl_kernel_zeus import moe_align_block_size_alloc as zeus_moe_align_block_size_alloc
        from sgl_kernel_zeus import moe_grouped_gemm as zeus_moe_grouped_gemm
        from sgl_kernel_zeus import silu_and_mul as zeus_silu_and_mul
        from sgl_kernel_zeus import moe_sum_reduce as zeus_moe_sum_reduce

        x_dev = dispatch_output.hidden_states.device
        assert (
            dispatch_output.topk_output.topk_weights.device == x_dev
            and dispatch_output.topk_output.topk_ids.device == x_dev
        ), (
            "UnquantizedFusedMoEMethod.forward_zeus: hidden_states / topk_weights / "
            "topk_ids must all be on the same device; got "
            f"hidden_states={x_dev}, "
            f"topk_weights={dispatch_output.topk_output.topk_weights.device}, "
            f"topk_ids={dispatch_output.topk_output.topk_ids.device}. "
            "Likely an upstream forward_zeus left an output on CPU — fix the "
            "upstream dispatcher (see TopK.forward_zeus pattern) instead of "
            "papering over here."
        )

        x          = dispatch_output.hidden_states          # [T, H] bf16
        topk_w     = dispatch_output.topk_output.topk_weights  # [T, K] fp32
        topk_ids   = dispatch_output.topk_output.topk_ids      # [T, K] i32

        T, H        = x.shape
        K           = topk_w.size(1)
        w13         = layer.w13_weight                      # [E, 2N, H] bf16
        w2          = layer.w2_weight                       # [E, H,  N] bf16
        E, two_N, _ = w13.shape
        N           = two_N // 2
        device      = x.device
        sum_scale = 1.0
        ...
        sorted_token_ids, expert_ids, num_tokens_post_pad = (
              zeus_moe_align_block_size_alloc(topk_ids, block_size=BLOCK_M, num_experts=E)
          )
        num_valid = T * K

        gemm1_out = torch.empty((T * K, two_N), dtype=torch.bfloat16, device=device)
        zeus_moe_grouped_gemm(
            A=x, B=w13, C=gemm1_out,
            sorted_token_ids=sorted_token_ids,
            expert_ids=expert_ids,
            num_tokens_post_pad=num_tokens_post_pad,
            num_valid_tokens=num_valid,
            top_k=K,                          # a_row = offs_token // K
            topk_weights=None,
            mul_routed_weight=False,
        )

        inter = zeus_silu_and_mul(gemm1_out)

        gemm2_out = torch.empty((T * K, H), dtype=torch.bfloat16, device=device)
        topk_w_bf16_flat = topk_w.to(torch.bfloat16).reshape(-1).contiguous()
        zeus_moe_grouped_gemm(
            A=inter, B=w2, C=gemm2_out,
            sorted_token_ids=sorted_token_ids,
            expert_ids=expert_ids,
            num_tokens_post_pad=num_tokens_post_pad,
            num_valid_tokens=num_valid,
            top_k=1,                          # 每个 routing entry 一行
            topk_weights=topk_w_bf16_flat,
            mul_routed_weight=True,
        )

        out = torch.empty((T, H), dtype=torch.bfloat16, device=device)
        zeus_moe_sum_reduce(
            input=gemm2_out.view(T, K, H),
            output=out,
            shared_output=None,
            routed_scaling_factor=sum_scale,
        )
