...
def _grouped_foreach_copy_(dsts: List[torch.Tensor], srcs: List[torch.Tensor]) -> None:
    ...
    for group_dsts, group_srcs in groups.values():
        if group_dsts and group_dsts[0].device.type == "zeus":
    ...
    def create(
        ...
        *,
        ...
        with torch.device(device):
            input_ids = torch.zeros((max_num_token,), dtype=index_dtype)
            input_embeds = torch.zeros((max_num_token, hidden_size), dtype=dtype)
            req_pool_indices = torch.zeros((max_bs,), dtype=index_dtype)
            ...
            positions = torch.zeros((max_num_token,), dtype=index_dtype)
            mrope_positions = torch.zeros((3, max_num_token), dtype=index_dtype)
...
class CudaGraphRunner:
    ...
    def capture_one_batch_size(
        ...
        forward_batch = ForwardBatch(
            ...
            # backends that do not yet have an int64 reduce kernel.
