...
class ModelRunnerKVCacheMixin:
    ...
    def profile_max_num_token(self: ModelRunner, pre_model_load_memory: int):
        ...
        effective_full_layer_ids = None
        effective_kda_layer_ids = None
        if self.is_draft_worker:
            ...
            effective_full_layer_ids = [
                i
                for i in mambaish.full_attention_layer_ids
                if self.start_layer <= i < self.end_layer
            ]
            effective_kda_layer_ids = [
                i
                for i in mambaish.linear_layer_ids
                if self.start_layer <= i < self.end_layer
            ]
            num_layers = len(effective_full_layer_ids)
        else:
            num_layers = self.num_effective_layers

        cell_size = self.get_cell_size_per_token(num_layers)

        reserved = pre_model_load_memory * (1 - self.mem_fraction_static)
        rest_memory = post_model_load_memory - reserved

        logger.info(
            f"[mem-debug] profile_max_num_token: "
            f"pre_model_load={pre_model_load_memory:.2f}GB, "
            f"post_model_load={post_model_load_memory:.2f}GB, "
            f"mem_fraction_static={self.mem_fraction_static}, "
            f"reserved=(1-frac)*pre={reserved:.2f}GB, "
            f"rest_before_mamba={rest_memory:.2f}GB, "
            f"cell_size_per_token={cell_size}B, "
            f"num_full_layers={num_layers}, "
            f"num_effective_layers={self.num_effective_layers}, "
            f"pp_range=[{self.start_layer},{self.end_layer}), "
            f"is_draft_worker={self.is_draft_worker}, "
            f"mambaish={self.mambaish_config is not None}"
        )
        if effective_kda_layer_ids is not None:
            logger.info(
                f"[mem-debug] profile_max_num_token (hybrid): "
                f"full_layer_ids={effective_full_layer_ids}, "
                f"kda_layer_ids={effective_kda_layer_ids}"
            )

        if self.mambaish_config is not None:
            rest_memory = self.handle_max_mamba_cache(rest_memory)

        profiled_tokens = int(rest_memory * (1 << 30)) // cell_size
        logger.info(
            f"[mem-debug] profile_max_num_token: "
            f"rest_for_full_kv={rest_memory:.2f}GB, "
    ...
    def handle_max_mamba_cache(self: ModelRunner, total_rest_memory):
        ...
        cache_per_req = config.mamba2_cache_params.mamba_cache_per_req
        logger.info(
            f"[mem-debug] handle_max_mamba_cache: "
            f"rest_in={total_rest_memory:.2f}GB, "
            f"mamba_full_memory_ratio={server_args.mamba_full_memory_ratio}, "
            f"mamba_cache_per_req={cache_per_req}B "
            f"({cache_per_req / (1 << 20):.2f}MB), "
            f"max_mamba_cache_size_arg={server_args.max_mamba_cache_size}, "
            f"max_running_requests_arg={server_args.max_running_requests}, "
            f"disable_radix_cache={server_args.disable_radix_cache}, "
            f"spec_algo={self.spec_algorithm}"
        )
        ...
        if not self.spec_algorithm.is_none():
            ...
            logger.info(
                f"[mem-debug] handle_max_mamba_cache: "
                f"spec_intermediate={mamba_state_intermediate_size / (1 << 30):.2f}GB, "
                f"rest_after_spec={total_rest_memory:.2f}GB"
            )
        ...
        rest_for_full_kv = total_rest_memory - mamba_state_memory
        logger.info(
            f"[mem-debug] handle_max_mamba_cache: "
            f"max_mamba_cache_size={server_args.max_mamba_cache_size}, "
            f"mamba_state_memory={mamba_state_memory:.2f}GB, "
            f"rest_for_full_kv={rest_for_full_kv:.2f}GB"
        )
        return rest_for_full_kv
    ...
    def _init_pools(self: ModelRunner):
        ...
        if self.server_args.attention_backend == "zeus":
            from sglang.srt.hardware_backend.zeus.memory_pool_zeus import (
                create_token_to_kv_pool,
            )

            self.token_to_kv_pool = create_token_to_kv_pool(self)
            ...
            # For hybrid models (e.g. GLM-next: KDA + NSA), only allocate KV
            # buffers for full-attention layers within this PP rank's range.
            attention_layer_ids = None
            nsa_layer_num = self.num_effective_layers
            if self.mambaish_config is not None:
                attention_layer_ids = [
                    i
                ...
                nsa_layer_num = len(attention_layer_ids)
        ...
        if self.token_to_kv_pool_allocator is None:
            if self.server_args.attention_backend == "zeus":
                from sglang.srt.hardware_backend.zeus.allocator_zeus import (
                    create_token_to_kv_pool_allocator,
                )

                self.token_to_kv_pool_allocator = create_token_to_kv_pool_allocator(
    ...
    def init_memory_pool(self: ModelRunner, pre_model_load_memory: int):
        ...
        avail_after = get_available_gpu_memory(self.device, self.gpu_id)
        logger.info(
            f"Memory pool end. "
            f"avail mem={avail_after:.2f} GB"
        )

        # Compact summary for hybrid (mambaish) models.
        if self.mambaish_config is not None:
            cache_per_req = self.mambaish_config.mamba2_cache_params.mamba_cache_per_req
            mamba_state_gb = (
                self.server_args.max_mamba_cache_size * cache_per_req / (1 << 30)
            )
            logger.info(
                f"[mem-debug] init_memory_pool summary (hybrid): "
                f"weight_load_mem={getattr(self, 'weight_load_mem_usage', float('nan')):.2f}GB, "
                f"max_total_num_tokens={self.max_total_num_tokens}, "
                f"max_running_requests={self.max_running_requests}, "
                f"max_mamba_cache_size={self.server_args.max_mamba_cache_size}, "
                f"mamba_state_alloc={mamba_state_gb:.2f}GB, "
                f"avail_after_init={avail_after:.2f}GB"
            )
