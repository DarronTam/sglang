...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    def prepare_encoder_info_extend(self, input_ids: List[int], seq_lens: List[int]):
        ...
        self.input_ids = torch.tensor(
            sum(input_ids, []),
            dtype=idx_dtype,
            pin_memory=_pin and idx_dtype == torch.int64,
        ).to(self.device, non_blocking=True)
    ...
    def filter_batch(
        ...
        self.seq_lens_sum = self.seq_lens_cpu.sum().item()  #TODO: can we maintain seq_lens_sum without summing after filtering?
