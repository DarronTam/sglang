...
def create_token_to_kv_pool_allocator(
    model_runner: "ModelRunner", need_sort: bool
) -> "ZeusPagedTokenToKVPoolAllocator":
    """Construct ZeusPagedTokenToKVPoolAllocator from a ModelRunner.

    Co-locates Zeus allocator construction with the implementation, so the
    mixin only carries the if-else dispatch (mirroring NPU's pattern).
    Requires ``model_runner.token_to_kv_pool`` to be already initialized.
    """
    return ZeusPagedTokenToKVPoolAllocator(
        model_runner.max_total_num_tokens,
        page_size=model_runner.page_size,
        dtype=model_runner.kv_cache_dtype,
        device=model_runner.device,
        kvcache=model_runner.token_to_kv_pool,
        need_sort=need_sort,
    )
