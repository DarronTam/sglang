...
def create_token_to_kv_pool(model_runner: "ModelRunner") -> "ZeusTokenToKVPool":
    """Construct ZeusTokenToKVPool from a ModelRunner.

    Mirrors the if-else dispatch pattern used by NPU in
    ``model_runner_kv_cache_mixin``, but keeps Zeus-specific construction
    kwargs co-located with the pool implementation rather than scattered
    across the mixin.
    """
    from sglang.srt.layers.dp_attention import get_attention_tp_size

    return ZeusTokenToKVPool(
