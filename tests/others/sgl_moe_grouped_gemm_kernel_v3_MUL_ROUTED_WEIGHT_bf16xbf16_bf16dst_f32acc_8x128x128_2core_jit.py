#!/usr/bin/env python3
"""Python-level Zeus JIT smoke for the v3 MUL_ROUTED_WEIGHT MoE grouped GEMM.

Reference kernel:
  /datau38020T/Application/caof/workspace/zeus514/triton_shared/python/
  zeus_examples/GEMM/
  sgl_moe_grouped_gemm_kernel_v3_MUL_ROUTED_WEIGHT_bf16xbf16_bf16dst_f32acc_8x128x128_2core.py

This variant models the gemm2 path: A is [T*topk, K], top_k is 1,
MUL_ROUTED_WEIGHT is 1, and routed weights are multiplied into the accumulator.
"""
...
@triton.jit
def sgl_moe_grouped_gemm_kernel_v3_MUL_ROUTED_WEIGHT_bf16xbf16_bf16dst_f32acc_8x128x128_2core(
    a_ptr,                     # *bf16 [T*topk, K]
    ...
    for m_block in range(total_m_blocks):
        ...
        for n_block in range(total_n_blocks):
            ...
            if off_expert == -1:
                ...
                w_inner = tl.arange(0, 1)
                w_ptrs = topk_weights_ptr + offs_token[:, None] + w_inner[None, :]
                routed_weight_2d = tl.load(
                    w_ptrs,
                    mask=valid_row[:, None],
                    other=0.0,
                )
                routed_weight_2d_f = routed_weight_2d.to(tl.float32)
...
def _ref_moe_grouped_gemm_v3_mul_routed_weight(
    A,
    B,
    sorted_token_ids,
    expert_ids,
...
def run_moe_grouped_gemm_v3_mul_routed_weight_jit_check() -> None:
    import torch
    ...
    sgl_moe_grouped_gemm_kernel_v3_MUL_ROUTED_WEIGHT_bf16xbf16_bf16dst_f32acc_8x128x128_2core[
        (CORE_NUM,)
    ](
    ...
    ref = _ref_moe_grouped_gemm_v3_mul_routed_weight(
        A,
        B,
        sorted_ids,
        expert_ids,
...
def main() -> int:
    run_moe_grouped_gemm_v3_mul_routed_weight_jit_check()
    print(
        "PASS: 2-core v3 MUL_ROUTED_WEIGHT sgl_moe_grouped_gemm Python-level Zeus JIT smoke"
    )
    return 0
